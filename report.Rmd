...
When it comes to the number of injuries, however, the number did not fluctuate as drastically as the number of death. It remained stable from 1900 to around1985. After hitting its pick at 10000 in around 1985, the number went down and continued to remain stable.
...
By conducting exploratory data analysis, we demonstrates the geographical and temporal distribution of volcanic eruptions. We find that most volcanoes are located along the **peripheries of continents**, especially along the **Pacific Ocean**. And while eruptions before 1000 A.D. seem to have a higher VEI than those after, it could be attributed to **reporting bias** from ancient era, in which only the most colossal volcanic eruptions were documented. We also explore the potential factors that influence the casualties and property damages from volcanic eruptions. From the numeric and descriptive data of volcanic losses, we get similar conclusions that the occurrence of **tsunami** and **earthquake** and the **VEI** of the eruption are **positively associated** with volcanic losses. And while there is a potential relationship between the **elevation** of a volcano and the deaths from its eruption, this association is not very significant and does not appear in other types of losses, hence requires further analysis.

For the statistical analysis, we choose **ordinal logistic regression** to assess the association between our predictors (tsunami, earthquake, VEI and elevation) and outcome (severity level of death). We perform a correlation analysis and find there is **no strong multicollinearity**, so we keep all the predictors and fit the ordinal logistic regression model: $volcano\_deaths{\_}description \sim flag\_earthquake + flag\_tsunami + volcanic\_explosivity\_index + elevation$. We calculate the p-values and CIs for the for the coefficient estimates, which suggest **significance** for all four predictors. Therefore, we calculate the odds ratios and draw the following conclusions about the factors influencing volcanic deaths:
...
While our study provides some insights into volcanic eruptions, it is worth noting that there are still many limitations to be recognized.

* Data quality: fail to record, ancient data reliability

* Data selection: None/No record data

* Model: generalizability, elevation
